...
%{
	...
	int complexity = 1;							// I use this to track the mccabe complexity
	float complexitySum = 0.0;  		// I use this to track the running sum of complexity
	int methodCount = 0;						// I use this to track the number of methods
	char getRank(int comp);					// Compute the rank based on complexity
	bool isClass = false;						// I use this to track if it's in a class
	char * className;								// Keeps track of class name

	std::vector<Keeper *> myVector;
...
funcdef // Used in: decorated, compound_stmt
	: DEF NAME parameters COLON suite
		...
		Keeper * myKeeper = new Keeper();
			if(isClass){
				myKeeper->setFuncName($2);
				complexitySum += complexity;
				++methodCount;
				myKeeper->setComplexity(complexity);
				myKeeper->setRowNo(@1.first_line);
				myKeeper->setColNo(@1.first_column);
				myKeeper->setRanking(getRank(complexity));
				myKeeper->setIsClass(isClass);
				myKeeper->setClassName(className);
				myKeeper->setBlockType('M');
				myVector.push_back(myKeeper);
...
classdef // Used in: decorated, compound_stmt
	: CLASS NAME LPAR opt_testlist RPAR COLON
		{isClass = true;className = new char[strlen($2) + 1];
		strcpy(className, $2);} suite
		{
			Keeper * theKeeper = new Keeper();
			if(methodCount == 0){
				theKeeper->setIsClass(isClass);
				theKeeper->setRowNo(@1.first_line);
				theKeeper->setColNo(@1.first_column);
				theKeeper->setClassName($2);
				theKeeper->setRanking(getRank(complexity));
				theKeeper->setComplexity(complexity);
				theKeeper->setBlockType('C');
				myVector.push_back(theKeeper);
			}else{
				float average = complexitySum / methodCount;
				theKeeper->setIsClass(isClass);
				theKeeper->setRowNo(@1.first_line);
				theKeeper->setColNo(@1.first_column);
				theKeeper->setClassName($2);
				theKeeper->setRanking(getRank(round(average)));
				theKeeper->setComplexity(round(average));
				theKeeper->setBlockType('C');
				myVector.push_back(theKeeper);
			}
			complexitySum = 0.0;
	...
	| CLASS NAME COLON {isClass = true;className = new char[strlen($2) + 1];
		strcpy(className, $2);} suite
		{
			Keeper * theKeeper = new Keeper();
			if(methodCount == 0){
